...
(defn- tx-present?
  ...
  is not annotated with `:import/tx-id` (meaning no skip check can be made)"
  [db tx-batch]
  (when-let [tx-uid (:import/tx-id (first tx-batch))]
    (let [tx-q-res (db.query/q+retry '[:find ?tx
                                       :in $ ?tx-id
                                       :where
                                       [?tx :import/tx-id ?tx-id]]
                                     db tx-uid)]
...
(defn diff-renames
  ...
  (let [append-diff-suffix (fn [import]
    ...
    (cons (-> tx-tx
              (update-in [:import/tx-id] append-diff-suffix)
         ...
         tx-xform (if-not import-name
                    ...
                    (let [db (d/db conn)
                          ...
                          base-xform (remove (fn [tx-batch]
                                               (when-let [batch-tx-id (-> tx-batch first :import/tx-id)]
