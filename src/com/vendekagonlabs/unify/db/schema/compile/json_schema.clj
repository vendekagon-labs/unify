...
(defn ->root-schema-definition [schema]
  (let [{:keys [unify.schema/version
    ...
    {:$schema "https://json-schema.org/draft/2020-12/schema"
     ;; TODO: troubleshoot (name name) coming out NULL in generated schema.
     :$id (format "https://www.unifybio.org/%s/%s/%s/schemas/import-config"
                  (namespace name) (name name) version)
     :title schema-info-str
     :description (str "A JSON schema generated by Unify, intended for use in validating yaml config "
                       "files dynamically in editors for this particular schema and version.")
     :type "object"}))
...
(defn value-attr-properties
  ...
  [value-attrs]
  ;; TODO: when card many, add unify-many special form to allowed types
  ;; via :oneOf key form.
  (->> value-attrs
...
(defn other-ref-properties
  ...
  [ref-attrs]
  ;; TODO: when card many, add unify many special form to property types
  ;; via :oneOf schema form
  (->> ref-attrs
